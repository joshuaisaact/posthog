...
export const productToursLogic = kea<productToursLogicType>([
    ...
    actions({
        ...
        // Element screenshot - store promise with step context
    ...
    listeners(({ actions, values }) => ({
        ...
        selectElement: ({ element }) => {
            ...
            if (isChangingExistingStep) {
                // Changing element for existing step - update step immediately, screenshot will be added when ready
                ...
                // Store promise so we can await it when saving
                actions.setPendingScreenshotPromise(stepIndex, screenshotPromise)

                // Stay in editing mode so user can save when ready
                actions.setEditorState({ mode: 'editing', stepIndex, stepType: 'element' })
            } else {
                // New step - store promise for confirmStep to await
                actions.setPendingScreenshotPromise(stepIndex, screenshotPromise)
        ...
        confirmStep: async ({ content, selector: selectorOverride, survey, progressionTrigger, maxWidth }) => {
            ...
            // Wait for pending screenshot if it's for this step
